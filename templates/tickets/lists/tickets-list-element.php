    ...
    <div class="p-10 calisia-tickets-row-content">
        ...
        <div class="calisia-ticket-row-title">
            <?php if($args['unread'] != 0){ ?>
                <strong>
            <?php } ?>
            <a href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ) . calisia_ticket_system\endpoint::get_ticket_endpoint_name(); ?>/?id=<?php echo $args['ticket']->get_model()->get_id(); ?>"><?php echo $args['ticket']->get_model()->get_title(); ?>
            <?php 
                if($args['unread'] != 0){
                    echo ' ('. $args['unread'] .')';
                } 
